...
本地寫會與排程搶 commit。所以以下三點要在**線上實際跑過**才算驗完。

1. **下一次 Paper Trade 執行後**（正常是 cloud-fetch 鏈跑完自動觸發，或 JST 23:00 死線）
   看 Telegram 日報應該出現：
   - 進場檔數 > 0（原本天天 0），持倉數往 13~15 移動、現金降到接近 0
   - 「⚠️ 額度滿錯過 N 檔合格候選」區塊（如果當天真的滿了）
   - 出場理由顯示中文（移動停損 / 時間停損 / 訊號失效）
2. **2026-08-26 前後**：既有 10 檔持倉有 7 檔已不在口袋名單，寬限期（5 個日曆日）過後
   應開始出現 `訊號失效` 出場。**若到 8/28 還一筆都沒有**，代表
   `last_on_list_date` 沒被寫進帳本，要查 Actions 有沒有正常 commit `auto-pocket.json`。
3. **反向防呆確認**：若某天口袋名單掃描壞掉（candidates=0），當天**不該**出現大量
   `訊號失效` 出場 — 程式有擋，但這是「壞掉才看得到」的路徑，順手確認一次。
