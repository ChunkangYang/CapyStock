...
- 現象：2026-07-21 ~ 08-21 共 26 個交易日，**進場 14 筆、出場 4 筆、22 天零動作**。
  07-24 滿倉後的 4 次進場日期完全等於 4 次出場日期 → 進場速率被出場速率 1:1 鎖死。
- 根因（皆為結構問題，非資料/排程故障）：
  1. `POSITION_JPY 300k × MAX_OPEN 10 = 3.0M = 起始資金 100%`，第 4 天滿倉；
     `select_new_trades` 額度檢查在迴圈最前面，之後進場邏輯實質停用。
  2. bot 帳本出場只有 `advance_trade` 的「−10% 棘輪移動停損」單一規則。
     config 已有的 `TIME_STOP_DAYS` / 三階段移動停損只接在 analyzer（watchlist）路徑，**bot 沒用到**。
     反事實實測（重現 production 4 筆出場）：改 15 日時間停損 → 12 筆；停損收到 5~7% → 7 筆；停利幾乎無效。
  3. 沒有「訊號失效出場」：08-21 的 10 檔持倉只有 3 檔還在口袋名單，7 檔早已不合格，
     同時 57 檔合格候選（含排名第 1 的 8141）全被「額度已滿」擋掉。7 天抽查皆同。
  4. 上限綁「檔數」不綁「資金」＋ lot 湊整 → 現金長期閒置 ¥470k~632k（16~21%）。
  5. 診斷盲點：滿倉後所有 skip 理由都被短路成「額度已滿」＋ `skipped[:50]` 截斷，
     Telegram 日報天天長一樣，所以一個月沒被發現。
- 附帶 bug（影響小）：舊排程下 `entry_date` 比進場價的收盤日晚一天，`advance_trade` 又跳過
  `d <= last_advanced_date` → **進場後第一根 K 棒永遠不被評估**。08-07 排程改 JST 18:00 後不再發生，邏輯仍在。
- 已排除：價格資料不新鮮（lag 0~1 日）、漏 K 棒、候選不足（43~80 檔且變多）、
  Actions 沒跑（origin 每交易日都有 commit，是**本地 checkout 落後 23 天**）。
- ❌ 待做（依效益排序）：訊號失效出場 → 時間停損 → 收緊停損 → 滿倉換倉 →
  資金上限取代檔數上限 → 日報加「今日錯過的機會」。
